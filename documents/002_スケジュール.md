| 大リリース（Major） | 目的（外面の節目） | 小リリース（Minor） | DoD（使える状態） | 主な機能  | 実装済|
| - | - | - | - | - | - |
| M1 | **CLIでDecision Pack（発注可能な数値）が出る** | m1.1 最小E2E（Coreのみ） | CLIで「ticker・buy_signal・entry/stop/size」が複数銘柄で出る（落ちない範囲で） | Universe固定 / yfinance OHLCV / features_daily最小 / Candidate最小 / Execution最小 / DecisionCore（最小）/ 標準出力 | 〇 |
