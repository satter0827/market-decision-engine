...
└── market_decision_engine/
    ...
    │
    ├── cli/                                # CLIエントリポイント（将来API化時に差し替え）
    │   ├── __init__.py
    │   ├── main.py                         # CLI起動点（typer/argparse）
    │   ├── args.py                         # CLI引数定義（必要なら分離）
    │   └── runners.py                      # JP/US EOD実行ラッパ（pipeline呼び出し）
    │
    ...
    │   ├── __init__.py
    │   ├── policy.py                       # UserPolicy / PolicySnapshot
    │   ├── decision.py                     # DecisionCore / DecisionPack（発注可能数値の契約）
    │   ├── features.py                     # Feature（日次/静的/財務）の契約
    │   ├── market.py                       # RunContext / MarketEnv / Regime 等（regimeのschemaはここ）
    │   └── reports.py                      # ShortSummary / Report の契約
    │
    ...
    │
    └── exceptions.py                       # 共通例外（Fatal/Degraded/Skip等の概念を集約）
